...
Description: Cortico-Basal Ganglia Network Model implemented in PyNN using the simulator Neuron.
            This version of the model loads the model steady state and implements DBS ampltiude
            modulation controllers where the beta ARV from the STN LFP is calculated at each
            controller call and used to update the amplitude of the DBS waveform that is applied
...
def generate_DBS_Signal(
    start_time, stop_time, dt, amplitude, frequency, pulse_width, offset
):
    ...
    DBS_Signal = (
        offset
        + 1.0
        * (1.0 + signal.square(2.0 * np.pi * frequency * tmp, duty=duty_cycle))
        / 2.0
    )
    DBS_Signal[-1] = 0.0
...
if __name__ == "__main__":
    ...
    # controller = Constant_Controller(SetPoint=1.0414e-04, MinValue=0.0, MaxValue=3.0, ConstantValue=2.5, Ts=0.02)		# Controller sampling period, Ts, is in sec
    # On-Off Controller:
    # controller = ON_OFF_Controller(SetPoint=1.0414e-04, MinValue=0.0, MaxValue=3.0, RampDuration=0.25, Ts=0.02)
    # Dual-threshold Controller:
    # controller = Dual_Threshold_Controller(LowerThreshold=1.0414e-04, UpperThreshold=1.5261e-04, MinValue=0.0, MaxValue=3.0, RampDuration=0.25, Ts=0.02)
    # P Controller:
    # controller = standard_PID_Controller(SetPoint=1.0414e-04, Kp=5.0, Ti=0, Td=0, Ts=0.02, MinValue=0.0, MaxValue=3.0)
    # PI Controller:
    controller = standard_PID_Controller(
        SetPoint=1.0414e-04, Kp=0.23, Ti=0.2, Td=0, Ts=0.02, MinValue=0.0, MaxValue=3.0
    )
    ...
    for controller_call_index, controller_call_time in enumerate(controller_call_times):
        ...
        STN_Pop.write_data(
            "/Simulation_Output_Results/Controller_Simulations/Amp/"
            + controller_label
            + "/STN_Pop/"
            + write_index
            + "STN_Soma_v"
    ...
    np.savetxt(
        "/Simulation_Output_Results/Controller_Simulations/Amp/"
        + controller_label
        + "/controller_amplitude_values.csv",
    ...
    w = neo.io.NeoMatlabIO(
        filename="/Simulation_Output_Results/Controller_Simulations/Amp/"
        + controller_label
        + "/STN_LFP.mat"
    )
